...
KernelKey cpu::ConvKeyGenerator::generate(const OpType &op_type, const DeviceType &device_type,
                                          const DataType &data_type, const int num_dim, const ConvType conv_type) {
    return baseKey(op_type, device_type, data_type) | reservedKey(num_dim, conv_type);
}
