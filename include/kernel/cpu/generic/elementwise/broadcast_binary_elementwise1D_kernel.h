...
namespace my_inference::cpu::generic {
    ...
    class BroadcastBinaryElementwise1DKernel : public BroadcastBinaryElementwiseKernelBase {
    public:
        explicit BroadcastBinaryElementwise1DKernel(const OpNode *op) : BroadcastBinaryElementwiseKernelBase(op) {
        }

        void operator()(const KernelParam &param) override {
            primitive::broadcastBinaryElementWise1D<T, op_type>(
                static_cast<T *>(param.inputs[0]), static_cast<T *>(param.inputs[1]),
                static_cast<T *>(param.outputs[0]), args_);
        }
    };
}
